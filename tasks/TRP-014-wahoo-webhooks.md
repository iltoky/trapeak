...
Статус: **Implemented; production configuration required**
...
1. [ ] Wahoo получает `200` на корректный `workout_summary` webhook.
2. [ ] Новая тренировка появляется в MCP без ручной синхронизации.
3. [ ] Повторная доставка не увеличивает число записей.
4. [ ] Изменение тренировки обновляет существующую запись.
5. [ ] Неверный token получает `401` и не меняет БД.
