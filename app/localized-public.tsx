...
function LocalizedFooter({ locale, compact = false }: Readonly<{ locale: AppLocale; compact?: boolean }>) {
  ...
  return <footer className={`shell ${compact ? "guide-footer" : ""}`}>
    <div className="footer-brand"><Link className="wordmark" href={localePath(locale)} aria-label={`TRAPEAK · ${common.home}`}><BrandLogo /></Link><p>{content.footerTagline}</p></div>
...
export function LocalizedProviderGuide({ locale, slug }: { locale: AppLocale; slug: ProviderGuideSlug }) {
  ...
  const l = publicCopy.guideLabels;
  const common = getUiMessages(locale).common;
  return <main className="guide-page"><SiteHeader locale={locale} publicPath={`/${slug}`} /><section className="guide-hero shell"><p className="guide-breadcrumb"><Link href={localePath(locale)}>TRAPEAK</Link><span>/</span><Link href={localePath(locale, "/ai-guides")}>{publicCopy.guideIndex.breadcrumb}</Link><span>/</span><strong>{c.eyebrow}</strong></p><div className="eyebrow"><span className="live-dot" />{c.eyebrow}</div><h1>{c.title}</h1><div className="guide-hero-bottom"><p>{c.intro}</p><div className="guide-status"><b>{l.productStatus}</b><span>{c.statusLabel}</span><p>{c.status}</p></div></div></section><section className="guide-answer"><div className="shell guide-answer-grid"><p className="section-index">{l.shortAnswer}</p><h2>{c.summary}</h2></div></section><section className="guide-section shell"><div className="guide-heading"><p className="section-index">{c.noIndex ? l.providerPlannedHow : l.providerHow}</p><h2>{l.providerTitle}</h2></div><ol className="guide-steps">{c.dataPoints.slice(0, 3).map((item, index) => <li key={item}><span>0{index + 1}</span><div><h3>{item}</h3></div></li>)}</ol></section><section className="guide-data"><div className="shell guide-data-grid"><div><p className="section-index light">{c.noIndex ? l.plannedData : l.dataAccess}</p><h2>{c.dataTitle}</h2><p>{c.dataIntro}</p></div><ul>{c.dataPoints.map((item, index) => <li key={item}><span>0{index + 1}</span>{item}</li>)}</ul></div></section><section className="guide-prompts shell"><div><p className="section-index">{l.examples}</p><h2>{l.askPlain}</h2></div><div className="prompt-list">{c.prompts.map((prompt) => <blockquote key={prompt}>“{prompt}”</blockquote>)}<small>{l.examplesNote}</small></div></section><section className="faq shell guide-faq"><p className="section-index">{common.faq}</p><h2>{publicCopy.landing.faqTitle}</h2>{c.faqs.map((faq) => <details key={faq.question}><summary>{faq.question}<span>+</span></summary><p>{faq.answer}</p></details>)}</section><section className="final-cta"><div className="shell"><BrandIcon className="cta-brand-icon" size={70} /><h2>{l.bring}</h2><p>{l.request}</p><a className="button gradient" href="mailto:support@trapeak.com?subject=TRAPEAK">{l.requestAccess} <span>↗</span></a></div></section><LocalizedFooter locale={locale} compact /></main>;
}

export function LocalizedLegalPage({ locale, path, content }: { locale: AppLocale; path: string; content: LegalContent }) {
  const publicCopy = getPublicContent(locale);
  const common = getUiMessages(locale).common;
  return <main className="legal"><SiteHeader locale={locale} publicPath={path} /><article><p className="section-index">{content.eyebrow}</p><h1>{content.title}</h1><p className="updated">{publicCopy.legal.updated}</p><p>{content.intro}</p>{content.sections.map((section) => <section key={section.title}><h2>{section.title}</h2>{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}{section.items ? <ol>{section.items.map((item) => <li key={item}>{item}</li>)}</ol> : null}</section>)}</article><footer className="shell"><div className="footer-brand"><BrandLogo /><p>{publicCopy.landing.footerTagline}</p></div><div><b>{common.contact.toUpperCase()}</b><a href="mailto:support@trapeak.com">support@trapeak.com</a></div><p className="copyright">{common.copyright}</p></footer></main>;
}
